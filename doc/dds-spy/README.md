...
The documentation on this page applies to the _DDS Spy_ Docker image with the `latest` tag, which refers to the most recent image released by RTI. To confirm the _Connext_ release that corresponds to the `latest` tag, or to review the other _Connext_ releases that support the _DDS Spy_ image, go to https://hub.docker.com/r/rticom/dds-spy/tags.

For documentation on previous releases of the _DDS Spy_ image, refer to the https://github.com/rticommunity/rticonnextdds-containers repository.

# Using the DDS Spy container image

Running _DDS Spy_ on Docker is as simple as running the `docker run` command:
...
This command starts _DDS Spy_ on domain ID 0.

The _DDS Spy_ container image uses the following user and group:
...
The above example runs _DDS Spy_ with the custom QoS configuration file.

## Command-line parameters

To provide your command-line parameters to _DDS Spy_, add
...
The above example runs _DDS Spy_ on domain ID 0 with the verbosity level set to 3.
