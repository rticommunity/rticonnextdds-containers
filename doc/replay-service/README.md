...
The documentation on this page applies to the _Replay Service_ Docker image with the `latest` tag, which refers to the most recent image released by RTI. To confirm the _Connext_ release that corresponds to the `latest` tag, or to review the other _Connext_ releases that support the _Replay Service_ image, go to https://hub.docker.com/r/rticom/replay-service/tags.

For documentation on previous releases of the _Replay Service_ image, refer to the https://github.com/rticommunity/rticonnextdds-containers repository.

## Using the Replay Service container image

Running _Replay Service_ on Docker is as simple as running the `docker run` command:

**Important:** The default configuration of _Replay Service_ expects database files generated by _Recording Service_ to be available inside the container. These database files contain the recorded data that _Replay Service_ will use. You must provide them (for example, a sample recording) using a bind mount or a Docker volume. Otherwise, the service will have no data to replay and will fail to start correctly with the default configuration.
...
| ------------------ | ------------------------------------------------------------------------------------------- |
| default            | This configuration replays all _Topics_ in domain 0. It records in XCDR_AUTO storage format. |
...
**Important**: The directory `$PWD/recording_service_database` must exist in the host machine, contain the database files in XCDR_AUTO format, and the user running the container (UID 1010, GID 1010) must have read and write permissions to it.
...
To provide your command-line parameters to _Replay Service_, add them to the end of the `docker run` command. For example:
...
The above example runs _Replay Service_ with the verbosity level set to WARN.
