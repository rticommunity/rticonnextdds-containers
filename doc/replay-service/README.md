...
For additional information on *RTI Replay Service*, refer to the [RTI documentation](https://community.rti.com/static/documentation/connext-dds/7.3.1/doc/manuals/connext_dds_professional/services/recording_service/replay/replay_index.html).
...
docker run -dt \
        --network host \
        -v $PWD/rti_license.dat:/opt/rti.com/rti_connext_dds-7.3.1/rti_license.dat \
        --name=replay_service \
        rticom/replay-service:latest \
        -cfgName default
```

This command starts *Replay Service* with an example configuration that replays a record that should be in the database files located in  ```/home/rtiuser/rti_workspace/7.3.1/database/```. The following sections explain the different ways to provide the database files.
...
```
-v $PWD/your/rti_license.dat:/opt/rti.com/rti_connext_dds-7.3.1/rti_license.dat
```
...
* ```/home/rtiuser/rti_workspace/7.3.1/user_config/replay_service```
...
```
docker cp replay_service:/opt/rti.com/rti_connext_dds-7.3.1/resource/xml/RTI_REPLAY_SERVICE.xml .
```
...
The default configuration takes the database files from ```/home/rtiuser/rti_workspace/7.3.1/database/cdr_recording```. Take into account that the database files are not in the container’s filesystem. To test this image, you can use the *RTI Recording Service* Docker image to record some data, and then use the *RTI Replay Service* image to replay it.
...
docker run -dt \
        --network host \
        -v $PWD/rti_license.dat:/opt/rti.com/rti_connext_dds-7.3.1/rti_license.dat \
        --mount type=volume,source=recording_service_database,target=/home/rtiuser/rti_workspace/7.3.1/database \
        --name=replay_service \
        rticom/replay-service:latest \
        -cfgName default
```
...
docker run -dt \
        --network host \
        -v $PWD/rti_license.dat:/opt/rti.com/rti_connext_dds-7.3.1/rti_license.dat \
        -v $PWD/recording_service_database:/home/rtiuser/rti_workspace/7.3.1/database \
        --name=replay_service \
        rticom/replay-service:latest \
        -cfgName default
```
...
```/home/rtiuser/rti_workspace/7.3.1/user_config/replay_service/USER_REPLAY_SERVICE.xml```
...
docker run -dt \
        --network host \
        -v $PWD/rti_license.dat:/opt/rti.com/rti_connext_dds-7.3.1/rti_license.dat \
        -v $PWD/MyReplayService.xml:/home/rtiuser/rti_workspace/7.3.1/user_config/replay_service/USER_REPLAY_SERVICE.xml \
        --name=replay_service \
        rticom/replay-service:latest \
...
```
docker cp replay_service:/opt/rti.com/rti_connext_dds-7.3.1/RTI_License_Agreement.pdf .
```
