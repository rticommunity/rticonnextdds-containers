...
The documentation on this page applies to the _Persistence Service_ Docker image with the `latest` tag, which refers to the most recent image released by RTI. To confirm the _Connext_ release that corresponds to the `latest` tag, or to review the other _Connext_ releases that support the _Persistence Service_ image, go to https://hub.docker.com/r/rticom/persistence-service/tags.

For documentation on previous releases of the _Persistence Service_ image, refer to the https://github.com/rticommunity/rticonnextdds-containers repository.
...
This command starts _Persistence Service_ with an example configuration that persists in memory all _Topics_ in domain 0 published by _DataWriters_ with TRANSIENT durability.
...
- User: rtiuser (UID 1010)
- Group: rtigroup (GID 1010)
...
| ------------------ | ----------------------------------------------------------------------------------------------- |
| default            | Persists in memory all _Topics_ published by _DataWriters_ with TRANSIENT durability.           |
| defaultDisk        | Persists in the file system all _Topics_ published by _DataWriters_ with PERSISTENT durability. |
...
**Important**: The directory `persistence_service_database` must exist in the host machine, and the user running the container (UID 1010, GID 1010) must have read and write permissions to it.
...
```shell
sudo chown -R 1010:1010 /var/lib/docker/volumes/persistence_service_database/_data
```
...
**Note:** The directory `persistence_service_database` must exist in the host machine, and the user running the container (UID 1010, GID 1010) must have read and write permissions to it.
